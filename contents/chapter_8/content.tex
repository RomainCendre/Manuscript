...
Ainsi, ce dernier chapitre propose une méthodologie en plusieurs étapes pour permettre la mise en place d'un processus d'aide au diagnostic multimodal en milieu clinique. Ce chapitre ne se focalise pas sur la robustesse des diverses modalités, pour lesquelles d'une part sont repris les propositions de la littérature propres aux modalités de photographie clinique et de dermatoscopie et d'autre part celle de ce manuscrit sur la \gls{rcm} formulés lors du \Cref{chap:chapter_7}. Par ailleurs, ce travail s'inspire pour certaines étapes de travaux réalisées sur la fusion de données.
...
\end{inlinerate} Dans la mesure où ce chapitre répond à une problématique binaire, seules les méthodes de calibration binaire sont considérées dont les méthodes \textbf{logistique} et \textbf{sigmoïde} sont les majeurs représentants. Ces modes de calibration sont recensés dans la \Cref{tab:multimodal_calibration_values} et leurs mise en œuvre se base sur la librairie logicielle \textit{Scikit-learn}~\cite{pedregosa2011}. 
